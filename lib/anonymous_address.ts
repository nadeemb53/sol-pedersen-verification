...
export class DKSAP implements AnonymousAddress {
  ...
  PublicKey(
    ...
    var shareHash = crypto
      .createHash("sha256")
      .update(shared1.toString("hex"))
      .digest("hex");
    var txHash = crypto
      .createHash("sha256")
      .update(message.toString("hex"))
      .digest("hex");
  ...
  Verify(
    ...
    var finalHash = crypto
      ...
      .digest("hex");
    let tmpKey = ec.keyFromPrivate(finalHash, "hex");
    let vPubkey = receiverPrivateKey
      .getPublic()
      .add(tmpKey.getPublic())
      .encode("hex");
