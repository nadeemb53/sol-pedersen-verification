...
contract EllipticCurve {
    ...
    function expMod(
        ...
        if (base == 0) return 0;
        if (e == 0) return 1;
        if (pp == 0) revert("Modulus is zero");
        r = 1;
        ...
        assembly {
            for {
                ...
                r := mulmod(
                    mulmod(r, r, pp),
                    exp(base, iszero(iszero(and(e, bit)))),
                    pp
                )
                r := mulmod(
                    mulmod(r, r, pp),
                    exp(base, iszero(iszero(and(e, div(bit, 2))))),
                    pp
                )
                r := mulmod(
                    mulmod(r, r, pp),
                    exp(base, iszero(iszero(and(e, div(bit, 4))))),
                    pp
                )
                r := mulmod(
                    mulmod(r, r, pp),
                    exp(base, iszero(iszero(and(e, div(bit, 8))))),
                    pp
                )
                bit := div(bit, 16)
            }
    ...
    function toAffine(
        ...
        uint256 z,
        uint pp
    ) internal pure returns (uint256 x2, uint256 y2) {
    ...
    function deriveY(
        ...
        // x^3 + ax + b
        uint256 y2 = addmod(
            mulmod(x, mulmod(x, x, pp), pp),
            addmod(mulmod(x, a, pp), b, pp),
            pp
        );
    ...
    function ecAdd(
        ...
        if (x1 == x2) {
            (x, y, z) = jacDouble(x1, y1, 1, a, pp);
        } else {
            (x, y, z) = jacAdd(x1, y1, 1, x2, y2, 1, pp);
        }
        // Get back to affine
        (qx, qy) = toAffine(x, y, z, pp);
    }
    ...
    function ecMul(
        ...
        (uint256 x1, uint256 y1, uint256 z1) = jacMul(d, x, y, 1, a, pp);
        // Get back to affine
        (qx, qy) = toAffine(x1, y1, z1, pp);
    }
    ...
    function jacAdd(
        ...
        if (zs[0] == zs[2]) {
            if (zs[1] != zs[3]) revert("Wrong data");
            else {
    ...
    function jacDouble(
        ...
        // m
        uint m = addmod(
            mulmod(3, square[0], pp),
            mulmod(a, mulmod(square[2], square[2], pp), pp),
            pp
        );
        // t
        ...
        // qy = -8*y1^4 + M(S-T)
        qy = addmod(
            mulmod(m, addmod(s, pp - qx, pp), pp),
            pp - mulmod(8, mulmod(square[1], square[1], pp), pp),
            pp
        );
        // qz = 2*y1*z1
