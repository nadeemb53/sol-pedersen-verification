...
describe("Secp256k1 Pedersen Commitment Test of Solidity Version", () => {
  ...
  it("should have same results of typescript and solidity", async () => {
    ...
    let soltC = await contract.commitWithH(
      r1,
      5n,
      BigNumber.from(H.x.toString()),
      BigNumber.from(H.y.toString())
    );
    ...
    let solAC2 = await contract.subCommitment(
      r2,
      BigNumber.from(aC1.x.toString()),
      BigNumber.from(aC1.y.toString()),
      r1,
      BigNumber.from(tC.x.toString()),
      BigNumber.from(tC.y.toString())
    );
    ...
    let solBC2 = await contract.addCommitment(
      r4,
      BigNumber.from(bC1.x.toString()),
      BigNumber.from(bC1.y.toString()),
      r1,
      BigNumber.from(tC.x.toString()),
      BigNumber.from(tC.y.toString())
    );
    ...
    const random = r1 + r4;
    console.log(
      "Gas cost for pedersen's commitment verification: ",
      await contract.estimateGas.verifyWithH(
        random,
        7n + 5n,
        bC2.x.toString(),
        bC2.y.toString(),
